 ...
 */
function get_slots_string($slot_value) {
	if ($slot_value == 0) {
		return "None";
   }
	...
	reset($dbslots);
	$slots = array();
	$special = array(2, 16, 512, 1024, 32768, 65536);
	$slot_bitmask = 1;
	for ($i = 0; $i <= 22; $i++) {
		if ($slot_bitmask <= $slot_value && $slot_bitmask & $slot_value) {
			if (!in_array($slot_bitmask, $special)) {
				array_push($slots, $dbslots[$slot_bitmask]);
			} else {
				if ($slot_value & ($special[0] + $special[1]) && !in_array("Ears", $slots)) {
					array_push($slots, "Ears");
				}
				
				if ($slot_value & ($special[2] + $special[3]) && !in_array("Wrists", $slots)) {
					array_push($slots, "Wrists");
				}
				
				if ($slot_value & ($special[4] + $special[5]) && !in_array("Fingers", $slots)) {
					array_push($slots, "Fingers");
				}
			}
		}
		$slot_bitmask *= 2;
...
function get_class_usable_string($class_value) {
	if ($class_value == 0) {
		return "None";
	} else if ($class_value == 65535) {
		return "All Classes";
	}
	...
	$class_bitmask = 1;
	for ($i = 0; $i <= 16; $i++) {
		if ($class_bitmask <= $class_value && $class_bitmask & $class_value) {
			$classes[$i] = $dbiclasses[$class_bitmask];
		}
		$class_bitmask *= 2;
	}
...
function get_race_usable_string($race_value) {
	if ($race_value == 0) {
		return "None";
	} else if ($race_value == 65535) {
		return "All Races";
	}
	...
	for ($i = 1; $i <= 16; $i++) {
        if ($race_bitmask <= $race_value && $race_bitmask & $race_value) {
            $races[$i] = $dbraces[$race_bitmask];
        }
        $race_bitmask *= 2;
    }
...
function get_deity_usable_string($deity_value) {
	global $dbideities;
	$deities = array();
	$deity_bitmask = 1;
	for ($i = 0; $i <= 17; $i++) {
		if ($deity_bitmask <= $deity_value && $deity_bitmask & $deity_value) {
			$deities[$i] = $dbideities[$deity_bitmask];
		}
		$deity_bitmask *= 2;
	}
	return implode(", ", $deities); 
}
...
function return_item_stat_box($item, $show_name_icon) {
	global $dbitypes, $dam2h, $dbbagtypes, $dbskills, $dbbardskills, $icons_url, $spells_table, $dbiaugrestrict, $dbiracenames;
	...
	/* Slots */
	if ($item["slots"]) {
		$html_string .= "<tr><td colspan='2'><b>Slot:</b> " . get_slots_string($item["slots"]) . "</td></tr>";
	}

	// Bag-specific information
	if ($item["bagslots"]) {
		$html_string .= "<tr><td width='0%' nowrap='1'><b>Type: </b>Container</td></tr>";
		$html_string .= "<tr><td width='0%' nowrap='1'><b>Bag Slots: </b>" . $item["bagslots"] . "</td></tr>";
		if ($item["bagtype"]) {
			$item_bag_type = $item["bagtype"];
			$bag_type = $dbbagtypes[$item_bag_type] != "" ? $dbbagtypes[$item_bag_type]  : "Unknown ($item_bag_type)";
	...
	$html_string .= "<table style='width: 150px; text-align: right;'>";
	$html_string .= "<tr><td><b>Size: </b></td><td>" . get_size_string($item["size"]) . "</td></tr>";
	$html_string .= get_item_stat_string("Weight", number_format(($item["weight"] / 10), 1));

	if ($dbitypes[$item["itemtype"]] != "" && $item["bagslots"] == 0) {
		if ($item["slots"]) {
			$html_string .= "<tr><td><b>Type:</b></td><td>" . $dbitypes[$item["itemtype"]];
		}
		$html_string .= "</td></tr>";
	}
	...
	$html_string .= get_item_stat_string("Backstab", number_format($item["backstabdmg"]));
	...
	if ($item["damage"] > 0) {
		switch ($item["itemtype"]) {
			...
			case 42: // H2H
				...
				$html_string .= "<tr><td><b>Bonus: </b></td><td>$dmgbonus</td></tr>";
				break;
			case 1: // 2hs
			case 4: // 2hb
			case 35: // 2hp
	...
	// Skill Mod
	if ($item["skillmodtype"] && $item["skillmodvalue"]) {
		$html_string .= "<tr><td colspan='2' nowrap='1'><b>Skill Modifier:</b> " . $dbskills[$item["skillmodtype"]] . " (" . $item["skillmodvalue"] . "%)</td></tr>";
	}
	
	// Augmentations
	for ($slot = 1; $slot <= 6; $slot++) {
		$augment_type = $item["augslot" . $slot . "type"];
		if ($augment_type) {
			$html_string .= "<tr>";
			$html_string .= "<td><img src='images/icons/blank_slot.gif' style='width:auto;height:10px'> <b>Slot $slot:</b> Type $augment_type</td></tr>";
		}
	}
	...
	if (($item["bardtype"] > 22) && ($item["bardtype"] < 65535)) {
		$item_bard_type = $item["bardtype"];
		$bard_type .= $dbbardskills[$item_bard_type] != "" ? $dbbardskills[$item_bard_type] : "Unknown ($item_bard_type)";
		
		$html_string .= "<tr><td width='0%' nowrap='1' colspan='2'><b>Bard Skill: </b>$bard_type";
		
		$bard_value = ($item["bardvalue"] * 10) - 100;
		if ($bard_value) {
			$html_string .= " ($bard_value%)</td></tr>";
		}
	}
	...
	if ($item["itemtype"] == 54) {
		if ($item["augtype"] > 0) {
			$aug_slots = array();
			$aug_type  = $item["augtype"];
			$aug_bitmask = 1;
			for ($i = 1; $i <= 30; $i++) {
				if ($aug_bitmask <= $aug_type && $aug_bitmask & $aug_type) {
					$aug_slots[$i] = $i;
				}
				$aug_bitmask *= 2;
			}
			$augment_slots_string = implode(", ", $aug_slots); 
			$html_string .= "<tr><td colspan='2' nowrap='1'><b>Augmentation Slots:</b> $augment_slots_string</td></tr>";
		} else {
			$html_string .= "<tr><td colspan='2' nowrap='1'><b>Augmentation Slots:</b> All Slots</td></tr>";
		}
	...
	if ($platinum || $gold || $silver || $copper) {
		$item_value .= "<tr><td><br><b>Value: </b>";
		$item_value .= $platinum ? "$platinum <img src='$icons_url\item_644.png' width='10px' height='10px'/> " : "";
		$item_value .= $gold ? "$gold <img src='$icons_url\item_645.png' width='10px' height='10px'/> " : "";
		$item_value .= $silver ? "$silver <img src='$icons_url\item_646.png' width='10px' height='10px'/> " : "";
		$item_value .= $copper ? "$copper <img src='$icons_url\item_647.png' width='10px' height='10px'/>" : "";
		$item_value   .= "</td></tr>";
		$html_string .= $item_value;
	}
...
function get_spell_name($spell_id) {
	global $spells_table;
	$query = "SELECT `name` FROM `$spells_table` WHERE `id` = '$spell_id'";
	$result = db_mysql_query($query);
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_array($result)) {
			return $row["name"];
...
function get_zone_long_name($zone_short_name) {
	global $zones_table;
	$query = "SELECT `long_name` FROM `$zones_table` WHERE `short_name` = '$zone_short_name'";
	$result = db_mysql_query($query);
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_array($result)) {
