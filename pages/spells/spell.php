...
for ($i = 1; $i <= 4; $i++) {
	$item_id = $spell["components" . $i];
	if ($item_id > 0) {
		$count = $spell["component_counts" . $i] ;
		$name = get_item_name($item_id);
		$icon = get_item_icon_from_id($item_id);
		$print_buffer .= "<tr>";
		$print_buffer .= "<td><b>Component $i</b></td>";
		$print_buffer .= "<td><a href = '?a=item&id=$item_id'>$icon $name</a> ($count Required)</td>";
		$print_buffer .= "</tr>";
	}
}
$print_buffer .= "</td></tr>";
...
if (mysqli_num_rows($result)) {
	$print_buffer .= "<h2 class='section_header'>Items with Spell Effect</h2><ul>";
